...
function menu.loadSaveCallback(_, filename)
	...
	local loadGame = function () LoadGame(filename) end
	if menu.uix_callbacks ["loadSaveCallback_preLoadGame"] then
		for uix_id, uix_callback in pairs (menu.uix_callbacks ["loadSaveCallback_preLoadGame"]) do
			loadGame = uix_callback(filename, loadGame)
		end
	end
	Helper.addDelayedOneTimeCallbackOnUpdate(loadGame, true, getElapsedTime() + 0.1)
...
-- kuertee end
	...
	if menu.uix_callbacks ["addSavegameRow_isListSaveGame"] then
		for uix_id, uix_callback in pairs (menu.uix_callbacks ["addSavegameRow_isListSaveGame"]) do
			...
			if not isAddSaveGameToList then  break  end
		end
        ...
        if menu.uix_callbacks[callbackName] then
            for _, updateData in ipairs(updateDatas) do
                if menu.uix_callbacks[callbackName][updateData.id] then
                    if Helper.isDebugCallbacks then
                        Helper.debugText_forced(menu.name .. " uix updateCallbacksNow (pre): menu.uix_callbacks[" .. tostring(callbackName) .. "][" .. tostring(id) .. "]: " .. tostring(menu.uix_callbacks[callbackName][updateData.id]))
                    end
