...
function newFuncs.registerCallback (callbackName, callbackFunction)
	...
	-- available callbacks:
	-- display_on_before_frame_display
	--
...
function newFuncs.display ()
	...
	local ownericon, owner, shiptrader, isdock, canbuildships, isplayerowned, issupplyship = GetComponentData(menu.currentcontainer, "ownericon", "owner", "shiptrader", "isdock", "canbuildships", "isplayerowned", "issupplyship")
	local cantrade = (C.IsComponentClass(menu.currentcontainer, "station") or C.IsComponentClass(menu.currentcontainer, "buildstorage") or issupplyship) and isdock
				...
				for i = 1, 10 do
					if groups[i] then
						...
						if C.IsComponentClass(menu.currentplayership, "station") then
							...
							local tradeactive = (groups[i].numassignableminingships == 0) and ((not usedassignments["trade"]) or (usedassignments["trade"] == i))
							table.insert(subordinateassignments, { id = "trade", text = ReadText(20208, 40101), icon = "", displayremoveoption = false, active = tradeactive, mouseovertext = tradeactive and "" or ReadText(1026, 8603) })
	...
	if callbacks ["display_on_before_frame_display"] then
		for _, callback in ipairs (callbacks ["display_on_before_frame_display"]) do
			callback (menu.frame)
		end
	end
